/**
 * IsohedralTiling Class
 * Complete implementation of the isohedral tiling algorithms
 * Uses the refactored data structure but maintains full compatibility with original API
 */
...
// Transformation matrices for U and S edge shapes
const TSPI_U = [
...
// mul and matchSeg functions now imported from '../utils/MathUtils.js'

export class IsohedralTiling {
    constructor(tilingType) {
        this.dataLoader = null;
        this.tilingTypeData = null;
        this.reset(tilingType);
    }
    ...
    reset(tilingType) {
        ...
        this.tiling_type = tilingType;
        this.tilingTypeData = this.dataLoader.getTilingType(tilingType);
        
        if (!this.tilingTypeData) {
            throw new Error(`Tiling type ${tilingType} is not defined in the library.`);
        }

        // Get the raw data arrays from the tiling type
        this.ttd = {
            num_params: this.tilingTypeData.getParameterCount(),
            num_aspects: this.tilingTypeData.getAspectCount(),
            num_vertices: this.tilingTypeData.getVertexCount(),
            num_edge_shapes: this.tilingTypeData.numEdgeShapes,
            edge_shapes: this.tilingTypeData.edgeShapes,
            edge_shape_ids: this.tilingTypeData.edgeShapeIds,
            edge_orientations: this.tilingTypeData.edgeOrientations,
            default_params: this.tilingTypeData.getDefaultParams(),
            vertex_coeffs: this.tilingTypeData.vertexCoeffs,
            translation_coeffs: this.tilingTypeData.translationCoeffs,
            aspect_coeffs: this.tilingTypeData.aspectCoeffs,
            colouring: this.tilingTypeData.coloring
        };
...
// Export utility functions as well for compatibility
export { mul, matchSeg, makePoint, makeMatrix };
