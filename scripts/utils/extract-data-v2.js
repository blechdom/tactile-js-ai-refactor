...
/**
 * Data Extraction Script for Tactile-JS v2
 * 
 ...
 * 
 * Handles the correct tiling type numbering (1-93 with gaps).
 */
 ...
 * 
 * Generated by extract-data-v2.js on ${new Date().toISOString()}
 * 
...
function verifyDataExtraction(data) {
    ...
    // Check that we have data for all valid tiling types
    data.validTilingTypes.forEach(typeNum => {
        const paddedNum = typeNum.toString().padStart(2, '0');
        const requiredArrays = [`es_${paddedNum}`, `esi_${paddedNum}`, `eo_${paddedNum}`, `dp_${paddedNum}`, `tvc_${paddedNum}`, `tc_${paddedNum}`, `ac_${paddedNum}`, `c_${paddedNum}`];
        
        requiredArrays.forEach(arrayName => {
            if (!data[getDataTypeFromName(arrayName)][arrayName]) {
                verification.errors.push(`Missing ${arrayName} for tiling type ${typeNum}`);
                verification.success = false;
            }
        });
    });

    // Check for extra data arrays (should be minimal)
    const allArrayNumbers = new Set();
    Object.keys(data.edgeShapes).forEach(name => {
        const num = parseInt(name.match(/\d+/)[0]);
        allArrayNumbers.add(num);
    });

    const extraArrays = Array.from(allArrayNumbers).filter(num => !data.validTilingTypes.includes(num));
    if (extraArrays.length > 0) {
        verification.warnings.push(`Extra data arrays found for numbers: ${extraArrays.join(', ')}`);
    }
...
const reportFile = path.join(__dirname, '..', '..', 'reports', 'data-extraction-report-v2.json');
const report = {
